...
struct Rollout
{
  Eigen::MatrixXd noise;            /**< @brief A matrix [num_dimensions][num_time_steps] of random noise applied to the
                                       parameters*/
  Eigen::MatrixXd parameters_noise; /**< @brief A matrix [num_dimensions][num_time_steps] of the sum of parameters +
                                       noise */

  Eigen::VectorXd state_costs;   /**< @brief A vector [num_time_steps] of the cost at each timestep */
  Eigen::MatrixXd control_costs; /**< @brief A matrix [num_dimensions][num_time_steps] of the control cost for each
                                    parameter at every timestep */
  Eigen::MatrixXd total_costs;   /**< @brief A matrix [num_dimensions][num_time_steps] of the total cost, where
                                    total_cost[d] = state_costs_ + control_costs_[d]*/
  Eigen::MatrixXd probabilities; /**< @brief A matrix [num_dimensions][num_time_steps] of the probability for each
                                    parameter at every timestep */
...
struct StompConfiguration
{
  ...
  int num_iterations_after_valid; /**< @brief Stomp will stop optimizing this many iterations after finding a valid
                                     solution */
  ...
  int max_rollouts; /**< @brief The combined number of new and old rollouts during each iteration shouldn't exceed this
                       value */

  // Cost calculation
  double control_cost_weight; /**< @brief Percentage of the trajectory accelerations cost to be applied in the total
                                 cost calculation >*/
};
