    ...
    const Eigen::VectorXd &currentJoints)
{
    ...
    // First, create a densified path in task space by interpolating between scan poses
    std::vector<Eigen::Affine3d> densifiedScanPoses;

    // Add the first pose
    densifiedScanPoses.push_back(originalScanPoses[0]);

    // Process remaining original poses and interpolate between them
    for (size_t i = 1; i < originalScanPoses.size(); ++i) {
        const auto &prevPose = originalScanPoses[i - 1];
        const auto &currentPose = originalScanPoses[i];

        // Determine distance metrics in task space
        double translationDistance = (currentPose.translation() - prevPose.translation()).norm();
        Eigen::Quaterniond q1(prevPose.linear());
        Eigen::Quaterniond q2(currentPose.linear());
        double rotationDistance = q1.angularDistance(q2);

        // Calculate number of intermediate steps needed
        const double MAX_TRANSLATION = 0.01; // 1cm max step
        const double MAX_ROTATION = 0.05;    // ~8.6 degrees max step

        int steps = 1; // Default: just add the target
        if (translationDistance > MAX_TRANSLATION || rotationDistance > MAX_ROTATION) {
            steps = std::max(static_cast<int>(std::ceil(translationDistance / MAX_TRANSLATION)),
                             static_cast<int>(std::ceil(rotationDistance / MAX_ROTATION)));
        }
        ...
        for (int step = 1; step <= steps; ++step) {
            ...
            Eigen::Vector3d interpolatedPosition = prevPose.translation() * (1.0 - t)
                                                   + currentPose.translation() * t;

            // Spherical interpolation for orientation
            Eigen::Quaterniond interpolatedRotation = q1.slerp(t, q2);
            ...
            interpolatedPose.translation() = interpolatedPosition;

            densifiedScanPoses.push_back(interpolatedPose);
        }
    }

    // Now process the densified path to find IK solutions
    std::vector<size_t> selectGoalPoseFallbacks; // Track where selectGoalPose was used
    
    if (!densifiedScanPoses.empty()) {
        // Find IK solution for the first pose
        auto [firstPoseArm, firstValid] = selectGoalPose(densifiedScanPoses[0]);
        ...
        // Process remaining densified poses
        for (size_t poseIdx = 1; poseIdx < densifiedScanPoses.size(); ++poseIdx) {
            const auto &pose = densifiedScanPoses[poseIdx];
            ...
            const int maxSmallVariationAttempts = 100;

            for (int attempt = 0; attempt < maxSmallVariationAttempts; ++attempt) {
                double q7Value = currentJointAngles[6] + dis(gen) * 0.1;
                ...
                try {
                    ...
                    Eigen::Matrix<double, 7, 1> prevEigen;
                    for (int i = 0; i < 7; i++) {
                        prevEigen[i] = currentJointAngles[i];
                    }
                    double jointDistance = (prevEigen - solEigen).norm();

                    double cost = jointDistance;
                    ...
                    if (cost < bestCost) {
                        ...
                        // Early exit if we find a very continuous solution
                        if (jointDistance < 0.1)
                            break;
                    }
            ...
            // If small variations fail, try selectGoalPose as the second approach
            if (!foundSolution) {
                qDebug() << "Small variations failed for pose" << poseIdx
                         << "- trying selectGoalPose";

                auto [fallbackArm, fallbackValid] = selectGoalPose(pose);

                if (fallbackValid && !armHasCollision(fallbackArm)) {
                    // Use the selectGoalPose solution and track as fallback
                    result.checkpoints.push_back(std::make_pair(fallbackArm, true));
                    selectGoalPoseFallbacks.push_back(poseIdx);
                    qDebug() << "Using selectGoalPose solution for pose" << poseIdx;

                    Eigen::Matrix<double, 7, 1> fallbackEigen = fallbackArm.getJointAngles();
                    for (int i = 0; i < 7; i++) {
                        currentJointAngles[i] = fallbackEigen[i];
                    }
                } else {
                    // If selectGoalPose fails or results in collision, mark as invalid
                    result.checkpoints.push_back(std::make_pair(result.checkpoints.back().first, false));
                    qDebug() << "selectGoalPose also failed or resulted in collision for pose"
                             << poseIdx << "- using previous pose as placeholder";
                }
            } else {
    ...
    if (!result.checkpoints.empty()) {
        // Find boundary points: invalid checkpoints and selectGoalPose fallbacks
        std::vector<size_t> boundaryIDs;
        
        // Track invalid checkpoints
        ...
        // Add selectGoalPose fallbacks as boundaries (excluding first pose)
        for (size_t fallbackIdx : selectGoalPoseFallbacks) {
            if (fallbackIdx > 0) { // Don't treat first pose as boundary
                boundaryIDs.push_back(fallbackIdx);
            }
        }
        ...
        if (boundaryIDs.empty()) {
            // No boundaries - entire sequence is one valid segment
            if (!result.checkpoints.empty()) {
            ...
            for (size_t boundaryIdx : boundaryIDs) {
                ...
                // Find next valid checkpoint after boundary
                ...
                if (nextValid < result.checkpoints.size()) {
                    // Create jump pair from last valid before boundary to first valid after
